...
from gaugegap.relativity.compton_schwarzschild import analyze_compton_schwarzschild
...
def main() -> int:
    ...
    # [information <-> time] Lieb-Robinson light cone (many-body speed limit)
    lr = analyze_lieb_robinson(n_sites=41, J=1.0)
    checks.append(("lieb_robinson_lightcone", lr.respects_cone))
    print(f"  [info<->time   ] Lieb-Robinson: front v={lr.front_velocity:.3f} <= "
          f"v_LR={lr.v_lr:.3f}  ok={lr.respects_cone}")

    # [mass <-> geometry] Compton-Schwarzschild: object radius floored by the Planck
    # length where the gravity and quantum boundaries cross (mass-radius diagram apex)
    m_planck = analyze_compton_schwarzschild(1.0).planck_mass
    cs = analyze_compton_schwarzschild(m_planck / np.sqrt(2.0))
    cs_floor = float(np.sqrt(2.0) * cs.planck_length)
    cs_ok = cs.identity_holds and abs(cs.min_allowed_radius - cs_floor) <= 1e-6 * cs_floor
    checks.append(("compton_schwarzschild_planck_floor", cs_ok))
    print(f"  [mass<->geo    ] Compton-Schwarzschild: R_s*lambda_C=2 l_P^2 (identity "
          f"{cs.identity_holds}); apex R={cs.min_allowed_radius:.3e} = sqrt2 l_P  "
          f"ok={cs_ok}")

    # [measurement <-> time] quantum Zeno effect: observation freezes evolution
    # (the rigorous physical cousin of the Hawthorne "being watched changes it" effect)
    Hz = np.array([[0, 1], [1, 0]], dtype=complex)
    psiz = np.array([1, 0], dtype=complex)
    zeno = analyze_quantum_zeno(Hz, psiz)
    zeno_ok = zeno.bound_respected and zeno.bound_increases and zeno.approaches_unity
    checks.append(("quantum_zeno_freezes", zeno_ok))
    ...
    payload = {
        ...
        "certificates": {
            ...
            "lieb_robinson_lean": lr.lean4, "lieb_robinson_coq": lr.coq,
            "compton_schwarzschild_lean": cs.lean4,
            "compton_schwarzschild_coq": cs.coq,
            "quantum_zeno_lean": zeno.lean4, "quantum_zeno_coq": zeno.coq,
        },
...
def _write_md(path, p, checks):
    lines = [
        ...
        f"| Lieb–Robinson cone | information ↔ time | x(t) ≤ v_LR·t + ξ | "
        f"{p['checks']['lieb_robinson_lightcone']} |",
        f"| Compton–Schwarzschild | mass ↔ geometry | R² ≥ R_s·λ_C = 2 ℓ_P² | "
        f"{p['checks']['compton_schwarzschild_planck_floor']} |",
        f"| Quantum Zeno | measurement ↔ time | survival floor 1−(ΔE·T)²/N ↑ 1 | "
        f"{p['checks']['quantum_zeno_freezes']} |",
        "",
