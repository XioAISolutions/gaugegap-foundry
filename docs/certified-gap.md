# The certified gap — one formula for every certificate
...
We name that functional the **certified gap**:
...
`src/gaugegap/certified_gap.py` makes `QG` first class:
...
- one generic discharged Lean 4 / Coq emitter, `emit_certified_gap_certificate`, proving
...
```bash
foundry run certifiedgap-0001-unified   # full report + emitted certificate
foundry run certifiedgap-smoke          # reduced deterministic run
```

Evidence lands in `results/certifiedgap-0001/` — a ledgered JSON bundle (`run_id`, `git_state`,
`config_hash`), a `rows.csv`, and the discharged `.lean` / `.v` certificate.

## Claim boundary

The certified gap is a **unifying bookkeeping functional over verified finite enclosures**. It
