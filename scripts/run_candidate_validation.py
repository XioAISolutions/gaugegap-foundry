...
def to_markdown(result: dict[str, object]) -> str:
    ...
    lines = [
        ...
        f"- QPY export available: `{qiskit_probe.get('qpy_export_available')}`",
        f"- Aer available: `{qiskit_probe.get('aer_available')}`",
        f"- IBM Runtime import available: `{qiskit_probe.get('ibm_runtime_available')}`",
        f"- Credentials checked: `{qiskit_probe.get('credentials_checked')}`",
