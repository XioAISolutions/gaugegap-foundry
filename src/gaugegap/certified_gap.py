"""The certified gap: one formula that unifies every certificate in the Foundry.
...
This module makes ``QG`` first class: a frozen :class:`CertifiedGap`, constructors that
...
class CertifiedGap:
    """A single certified (or refuted) certified gap ``QG(O, β) = lower(O) − β``."""
    ...
    @property
    def gap(self) -> float:
        """The certified gap: verified lower endpoint minus the floor."""
    ...
    def certificate(self) -> tuple[str, str]:
        ...
        return emit_certified_gap_certificate(self.observable, self.enclosure_low, self.floor)
...
def emit_certified_gap_certificate(label: str, low: float, floor: float) -> tuple[str, str]:
...
/-- The certified gap QG = lo − beta is positive (no holes). -/
theorem certified_gap_pos : lo - beta > 0 := by
...
Theorem certified_gap_pos : lo - beta > 0.
Proof. lra. Qed.
...
# --------------------------------------------------------------------------- #
# The end-to-end run: one representative certified CertifiedGap per track.
# --------------------------------------------------------------------------- #

def unified_gap_report() -> dict[str, object]:
    """Compute one representative CertifiedGap per track through the *same* formula.
