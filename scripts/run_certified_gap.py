#!/usr/bin/env python3
"""Run the unified certified gap report (certifiedgap-0001) -- the program end to end.
...
The certified gap is a unifying bookkeeping functional over the repository's verified
...
from gaugegap.certified_gap import (  # noqa: E402
    CLAIM_BOUNDARY,
    emit_certified_gap_certificate,
    unified_gap_report,
)

HYPOTHESIS_ID = "certifiedgap-0001"
...
def main(argv: list[str] | None = None) -> int:
    ...
    csv_lines = ["track,observable,enclosure_low,floor,certified_gap,certified"]
    ...
    if args.emit_certificate:
        lean, coq = emit_certified_gap_certificate("unified", 1.0, 0.0)
        (out / f"{HYPOTHESIS_ID}_gap.lean").write_text(lean)
        (out / f"{HYPOTHESIS_ID}_gap.v").write_text(coq)

    print("=" * 72)
    print(f"CertifiedGap {HYPOTHESIS_ID}: the unified formula, one row per track")
