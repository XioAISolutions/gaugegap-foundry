...
def main() -> int:
    ...
    parser.add_argument("--command", action="append", default=None, help="Extra command to run; can be repeated")
...
def _run_command(command: list[str], output_dir: Path) -> dict[str, object]:
    ...
    (command_dir / f"{command_hash}.stdout.txt").write_text(proc.stdout, encoding="utf-8")
    (command_dir / f"{command_hash}.stderr.txt").write_text(proc.stderr, encoding="utf-8")
    return {
        "command": command,
        "returncode": proc.returncode,
        "started_at_utc": started.isoformat(),
        "ended_at_utc": ended.isoformat(),
        "stdout_path": str((command_dir / f"{command_hash}.stdout.txt").relative_to(output_dir)),
        "stderr_path": str((command_dir / f"{command_hash}.stderr.txt").relative_to(output_dir)),
...
def _split_command(command: str) -> list[str]:
    # Keep this intentionally simple. For complex shell commands, create a script.
