...
Selection score and margin thresholds are chosen on a disjoint calibration split,
committed, then frozen before unseen test seeds run. A deceptive no-fit case keeps
the target model above the old 0.70 gate while violating one registered
prediction, making threshold calibration consequential.
"""
...
class CalibratedPendulumChallengeExecutive(SealedPendulumChallengeExecutive):
    """Add a high-confidence no-fit case used to calibrate abstention gates."""
    ...
    def __init__(self, *, case_kind: str, **kwargs: Any) -> None:
        ...
        base_kind = CASE_RATIO_SUPPORTED if case_kind == CASE_DECEPTIVE_NO_FIT else case_kind
        config = kwargs.get("config")
        if case_kind == CASE_DECEPTIVE_NO_FIT:
            config = config or ExecutiveConfig(confidence_threshold=0.70)
            # The deceptive contradiction lives in the repeat measurement. Do not
            # permit an earlier evidence gate to stop before that committed probe.
            kwargs["config"] = replace(config, early_stop=False)
        ...
        return {
            ...
            "perturbation": {
                ...
                "purpose": (
                    ...
                    "violating one registered prediction"
                ),
            },
        }
    ...
    def _perturb_repeat(self, observation: Observation) -> Observation:
        ...
        readings["amplitude_correction_s"] = readings["period_s"] - readings["small_angle_period_s"]
        return replace(
            observation,
    ...
    def _run_intervention(self, action, salience_score: float, index: int) -> PendulumOutcome:
        ...
        if not (
            ...
            return outcome
        observation_b = self._perturb_repeat(outcome.observation_b)
...
def _record_from_artifact(
    artifact: dict[str, Any], *, split: str, policy: str
) -> dict[str, Any]:
...
def _annotate(
    ...
    *,
    ...
    artifact["calibration_protocol"] = {
        ...
        "calibration_record_hashes": list(record_hashes),
...
def _policy_summary(rows: list[dict[str, Any]], policy: str) -> dict[str, Any]:
    ...
    return {
        ...
        "coverage": round(len(accepted) / len(decisions), 6) if decisions else None,
        "selective_accuracy": _rate(accepted, "correct"),
        "mean_top_score": round(fmean(float(item["top_score"]) for item in decisions), 6)
        if decisions
        else None,
...
def _reliability_bins(rows: list[dict[str, Any]], policy: str) -> list[dict[str, Any]]:
    decisions = [row[policy] for row in rows]
    result = []
    for low, high in ((0.0, 0.75), (0.75, 0.85), (0.85, 0.95), (0.95, 1.000001)):
        items = [item for item in decisions if low <= float(item["top_score"]) < high]
        result.append(
            {
                "lower": low,
                "upper": round(high, 6),
                ...
                "acceptance_rate": (
                    round(fmean(0.0 if item["abstained"] else 1.0 for item in items), 6)
                    if items
                    else None
                ),
            }
    ...
    *,
    ...
    for seed in calibration_seed_list:
        for case_kind in CALIBRATION_CASES:
            ...
            records.append(_record_from_artifact(artifact, split="calibration", policy="cold"))
            if root is not None:
    ...
    threshold, scorecards = choose_frozen_threshold(
        records,
    ...
    for seed in test_seed_list:
        ...
        for case_kind in CALIBRATION_CASES:
            configs = {
                "cold": (ExecutiveConfig(use_salience=True, early_stop=True, confidence_threshold=0.70), None, []),
                "warm": (ExecutiveConfig(use_salience=True, early_stop=True, confidence_threshold=0.70), trained_memory, parents),
                "fixed": (ExecutiveConfig(use_salience=False, early_stop=True, confidence_threshold=0.70), None, []),
            }
            policy_records: dict[str, Any] = {}
            for policy, (config, memory, policy_parents) in configs.items():
                artifact = CalibratedPendulumChallengeExecutive(
                    case_kind=case_kind,
                    seed=seed,
                    config=config,
                    salience_memory=memory,
                    parent_artifact_hashes=policy_parents,
                ...
                record = _record_from_artifact(artifact, split="test", policy=policy)
                ...
                frozen.update(
                    {
                        ...
                        "decision_matches_frozen": (
                            actual.get("selected_outcome") == frozen["selected_outcome"]
                ...
                if root is not None:
                    path = root / "test" / f"seed-{seed}" / case_kind / f"{policy}.eja.json"
                    dump_artifact(artifact, path)
                    index.append(
                        {
                            "split": "test",
            ...
            rows.append({"seed": seed, "case_kind": case_kind, **policy_records})
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--report", default="artifacts/jump-lab-calibration/calibration-suite.json"
    )
    parser.add_argument(
        "--html", default="artifacts/jump-lab-calibration/calibration-suite.html"
    )
    ...
    if args.html:
        ...
        html_path.write_text(render_calibration_html(report), encoding="utf-8")
