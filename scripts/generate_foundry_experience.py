...
<aside id="left">
  ...
  <div class="section"><div class="label">Equation controls</div><div id="params"></div><button id="run">Re-integrate finite model</button><button id="proofpack" style="width:100%;margin-top:8px">Export proofpack</button></div>
...
// Browser proofpack export: schema gaugegap.browser_attractor_proofpack.v1.
// Bundles the live slider parameters, solver settings, trajectory digest and
// claim boundary into a downloadable, content-hashed JSON evidence file.
function canonicalJson(v){if(Array.isArray(v))return '['+v.map(canonicalJson).join(',')+']';if(v&&typeof v==='object')return '{'+Object.keys(v).sort().map(k=>JSON.stringify(k)+':'+canonicalJson(v[k])).join(',')+'}';return JSON.stringify(v===undefined?null:v)}
async function sha256Hex(str){try{const buf=await crypto.subtle.digest('SHA-256',new TextEncoder().encode(str));return Array.from(new Uint8Array(buf)).map(b=>b.toString(16).padStart(2,'0')).join('')}catch(e){return 'unavailable-no-webcrypto'}}
async function buildProofpack(sc,traj){const steps=sc.id==='thomas'?18000:12000,burn=sc.id==='thomas'?4000:2500;const rounded=traj.map(p=>p.map(v=>Number(v.toFixed(7))));const pack={schema:'gaugegap.browser_attractor_proofpack.v1',created_at_utc:new Date().toISOString(),page:{schema:DATA.schema,git_commit:DATA.git_commit||null,title:document.title},scene:{id:sc.id,label:sc.label,equations:sc.equations||[]},solver:{method:'RK4',dt:sc.dt,steps:steps,burn_in:burn,sample_every:4,initial_state:sc.defaults},parameters:attractorParams[sc.id]||{...sc.parameters},display:{projection:projection.value,density:+density.value,speed:+speed.value,trail:+trail.value},precomputed_diagnostics:sc.diagnostics||null,trajectory:{points:rounded.length,first:rounded[0]||null,last:rounded[rounded.length-1]||null,sha256:await sha256Hex(canonicalJson(rounded))},claim_boundary:{scene:sc.claim_boundary||null,global:DATA.claim_boundary},attribution:DATA.attribution||null,notes:'Browser floating-point results (including Math.sin) are implementation-defined; trajectory hashes are reproducible per browser build, not across engines.'};pack.content_hash=await sha256Hex(canonicalJson(pack));return pack}
document.getElementById('proofpack').onclick=async()=>{const sc=current(),btn=document.getElementById('proofpack');if(sc.kind!=='attractor'){btn.textContent='Proofpack: attractor scenes only';setTimeout(()=>{btn.textContent='Export proofpack'},2200);return}btn.textContent='Building proofpack…';try{const traj=integrateClient(sc);points=traj;frame=0;updatePanels();const pack=await buildProofpack(sc,traj);const blob=new Blob([JSON.stringify(pack,null,2)],{type:'application/json'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='gaugegap-proofpack-'+sc.id+'-'+String(pack.content_hash).slice(0,8)+'.json';a.click();URL.revokeObjectURL(a.href)}finally{btn.textContent='Export proofpack'}};
